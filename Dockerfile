...
ENV VERSION=v4.2.6 NPM_VERSION=2
...
RUN apk add --update openssl pkgconfig curl make gcc g++ binutils-gold python linux-headers paxctl libgcc libstdc++ && \
...
RUN cd /node-${VERSION} && \
  ...
  if [ -x /usr/bin/npm ]; then \
    npm install -g npm@${NPM_VERSION} ngn@0.2.131 && \
